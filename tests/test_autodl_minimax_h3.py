...
@pytest.mark.asyncio
async def test_remote_catalog_resolution_includes_local_autodl(monkeypatch):
    from novelvideo.api.routes import freezone

    async def remote_catalog(_media_type):
        return [{"id": "newapi_seedance-2.0", "apiModel": "newapi_seedance-2.0"}]

    monkeypatch.setattr(freezone, "_ee_media_model_catalog", remote_catalog)
    schema, params, capabilities = await freezone._resolve_catalog_request(
        "video",
        AUTODL_MINIMAX_H3_IMAGE_REFERENCE_BACKEND,
        {},
        mode="imageReference",
    ...
    assert schema == {}
    assert params == {}
    assert capabilities["providerId"] == "autodl"
    assert capabilities["supportedModes"] == ["image_reference"]
...
def test_autodl_backend_has_explicit_billing_model():
    ...
    assert (
        _video_backend_cost_model(AUTODL_MINIMAX_H3_IMAGE_REFERENCE_BACKEND)
        == "minimax-h3-local"
    )
